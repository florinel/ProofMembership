# Copilot instructions for ProofMembership
...
- The current web app is backed by a file-based local projection instead of a database. `apps/web/src/lib/data/store.ts` writes the read model to `.proofmembership/indexer/read-model.json`, mirrors transitions to `.proofmembership/indexer/events.json`, and stores both local synthetic and RPC-verified on-chain purchase projections served through `/api/metadata/[assetId]`.
- Uploaded campaign artwork is stored under `.proofmembership/media`. `apps/web/src/lib/media/mediaStore.ts` persists the uploaded file plus a small JSON sidecar, and `/api/media/[mediaId]` serves that content back to the app.
...
- `apps/web/tsconfig.json` maps `@proofmembership/types` and `@proofmembership/sdk` directly to workspace source files. When changing shared types, update the packages first and let the app compile against source.
- Auth is split into two tracks: a development-only role cookie for local UI work and a wallet-challenge flow that issues a signed session cookie. The server resolves role from `PROOFMEMBERSHIP_ADMIN_WALLETS`, existing club ownership, and existing non-revoked memberships in the current read model.
- Storefront `PROOFMEMBERSHIP_PURCHASE_MODE=onchain` now builds the real Anchor `purchase_membership` instruction in the browser wallet path, then confirms via `/api/storefront/purchase/confirm`, which validates the transaction from RPC before projection.
...
- Reuse `@proofmembership/types` instead of creating local copies of campaign, membership, or asset metadata types in the web app.
